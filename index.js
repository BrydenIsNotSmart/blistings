...
fs.readdir("./src/commands", (err, files) => {
    ...
    console.log(`[BListings.xyz]: ${files.length} command loaded.`);
    files.forEach(f => {
...
client.channels.cache.get('852580839103660056').send(`<a:leave:870482797142376469> \`${member.user.tag}\` left the server.`)
  client.channels.cache.get('852580839103660060').send(`<a:leave:870482797142376469> \`${member.user.tag}\` left the server.`)
...
client.on("guildMemberAdd", async (member) => {
  ...
  client.channels.cache.get('852580839103660056').send(`<:memberadd:870482431801704478> \`${member.user.tag}\` joined the server!`)
  client.channels.cache.get('852580839103660060').send(`<:memberadd:870482431801704478> \`${member.user.tag}\` joined the server!`)
});
/*=======================================================================================*/
...
client.on('ready',async () => {
    console.log("[BListings.xyz]: Bot successfully connected as "+client.user.tag+".");
    let botsSchema = require("./src/database/models/botlist/bots.js");
    const bots = await botsSchema.find();
    client.user.setPresence({ activity: { type: 'WATCHING', name: 'BListings.xyz | '+bots.length+' bots!' }, status: "online" });
});
/*=======================================================================================*/

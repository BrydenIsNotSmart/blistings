  ...
  module.exports = async (client) => {
   ...
   app.get("/login", (req, res, next) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://www.blistings.xyz/user/${a.id}) The user named **site** tried to log in but could not log in because he was blocked from the site. `).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
        })
        req.session.destroy(() => {
        res.json({ login: false, message: "You have been blocked from BListings.", logout: true })
        req.logout();
        });
        } else {
            try {
        ...
        client.users.fetch(req.user.id).then(async a => {
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://www.blistings.xyz/user/${a.id}) User named **site** logged in.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    http.listen(3000, () => { console.log("[BListings.xyz]: Website running on 3000 port.")});
  ...
  const checkMaintence = async (req, res, next) => {
    ...
    app.get("/admin/premium/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.botID).then(bota => {
            ...
            client.users.cache.get(botdata.ownerID).send(`Your bot  **${bota.tag}** has been **promoted**.`)
        });
...
app.post("/admin/maintence", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(global.config.server.channels.webstatus).send(`<:offline:871902982566993920> BListings has been switched to __maintenance__ due to **${req.body.reason}**.`).then(a => {
        new maintenceSchema({
            server: config.server.id,
...
app.post("/admin/unmaintence", global.checkAuth, async (req, res) => {
    ...
    const bakimsonaerdikardesvcodes = new dc.MessageEmbed()
        .setAuthor("blisitngs.xyz", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`<:online:871558291564679259> BListings is **active** again!\n[Click to redirect website](https://www.blistings.xyz)`)
        .setFooter("BListings © All rights reserved.");
    await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
        a.edit(`~~ <:online:871558291564679259> BListings has been switched to __maintance__ due to **${bakimdata.reason}**. ~~`, bakimsonaerdikardesvcodes)
    })
    ...
    app.get("/admin/premium/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.botID).then(bota => {
            client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>'s bot **${bota.tag}**'s premium has been removed.`)
            client.users.cache.get(botdata.ownerID).send(`Your bot  **${bota.tag}**'s premium has been removed.`)
        });
    ...
    sleep(1050)
    console.clear();
    console.log('\x1b[36m%s\x1b[0m', "[BListings.xyz]: General routers loading...");
    sleep(500);
    ...
    /* Uptime System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[BListings.xyz]: Uptime system routers loading...");
    sleep(500);
    ...
    /* Profile System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[BListings.xyz]: Profile system routers loading...");
    sleep(500);
    ...
    /* Code Share System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[BLisitngs.xyz]: Code Share system routers loading...");
    sleep(500);
    ...
    /* Botlist System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[BListings.xyz]: Botlist system routers loading...");
    sleep(500);
    ...
    /* Server List System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[BListings.xyz]: Serverlist system routers loading...");
    sleep(500);
    ...
    app.use(async (req, res, next) => {
       if(req.path.includes('/admin')) {
        if (req.isAuthenticated()) {
          if(client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(global.config.server.roles.administrator) || client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(global.config.server.roles.moderator) || req.user.id === "529815278456930314") {
              next();
              } else {
              res.redirect("/error?code=403&message=You is not competent to do this.")
          }
    ...
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[BListings.xyz]: Admin Panel system routers loading...");
    sleep(500);
    ...
    /* Bot System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[BListings.xyz]: Bot system loading...");
